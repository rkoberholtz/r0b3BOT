...
async def on_ready():
    ...
    while True:
        ...
        if print_completion != "Unknown" and print_completion < 100:
            ...
            # Will hit this if the last print job completed and is sitting at 100%
            print(">>>> Nothing Printing.")
...
async def updateStatus():
    ...
    while print_completion < 100:
        try:
            # Try to get the % of Completion
            print_completion = round(octoapi.get_completion(), 1)
        except:
        ...
        if print_completion != 999:
            unknowns = 0
            print(f"  - Print job is at {print_completion}%, with {time_left} to go")
            activity = discord.Activity(name=f"3D Print @ {str(print_completion)}%, {time_left} to go",type=discord.ActivityType.watching)
        else:
            unknowns += 1
